...
/// ЗАДАЧА 27: Выбери напиток -> определим тип меню
public func task27() {
    printHeader("Задача 27: Выбери напиток -> определим тип меню")
    
    print("\n🍽️  ВЫБЕРИТЕ БЛЮДО")
    print("1 - Салаты (Salads)")
    print("2 - Супы (Soups)")
    print("3 - Основные блюда (Main Dishes)")
    print("4 - Напитки (Drinks)")
    
    // 1. Enter data
    guard let dish = safeIntInput(prompt: "Enter number of dish(1-4):"),
          dish != 4 else {
        print("О, ты предпочитаешь вегетерианское меню!")
        pressEnterToContinue()
        return
    }
